...
export function TeamConnectionsDialog({connections,personalConnections,onCopy,onClose}:{connections:RegistryConnection[];personalConnections:SavedConnection[];onCopy:(id:string)=>void;onClose:()=>void}) {
  ...
  return <div className="dialog-backdrop"><section ref={dialogRef} className="dialog team-dialog" role="dialog" aria-modal="true" aria-label="复制团队共享连接">
    ...
    {showCompletion?<div className="team-completion"><Icon name="check"/><h3>团队连接均已添加到个人列表</h3><p>当前没有待复制连接，可以查看已经添加的连接。</p><button className="oc-button" onClick={()=>setStatus('copied')}>查看已在个人</button></div>:<div className="team-table-wrap">
    ...
    <footer className="team-dialog-footer"><span>仅复制配置到个人列表；密码仍由浏览器加密保存。</span><div><button className="oc-button" onClick={onClose}>关闭</button>{!showCompletion&&<button className="oc-button primary" disabled={!selected.length} aria-label={`复制选中（${selected.length}）`} onClick={copySelected}>复制选中（{selected.length}）</button>}</div></footer>
  </section></div>
}
