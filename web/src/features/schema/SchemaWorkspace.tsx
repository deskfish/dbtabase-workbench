...
export function SchemaWorkspace({api,connectionId,schema,table,driver,onSaved}:{api:APIClient;connectionId:string;schema:string;table:string;driver:'mysql'|'postgres';onSaved:()=>void}){
 ...
 return <section className="schema-workspace" aria-label="表结构编辑">
  ...
  <div className="schema-body">
   ...
   <aside className="ddl-panel"><h3>SQL 预览</h3>{previewBusy&&operations.length>0?<p className="ddl-preview-hint">正在生成 SQL…</p>:preview?<><pre>{preview.statements.map(x=>x.sql+';').join('\n\n')}</pre>{preview.warnings?.map(x=><p className="ddl-warning" key={x}>{x}</p>)}{preview.risks.map(x=><p className={`ddl-risk ${x.level}`} key={x.kind+x.target}>{x.message}</p>)}</>:<p>{operations.length>0?'等待生成 SQL 预览…':'修改字段、索引或关系后，SQL 会在这里实时显示。'}</p>}</aside>
  </div><footer className="schema-status">{busy?'正在保存…':previewBusy?'正在更新 SQL 预览…':message||'修改会实时生成 SQL 预览，确认后点击保存'}</footer>
 </section>
}

function IndexEditor({indexes, columnOptions, onAdd, onEdit, onDrop}: {indexes: SchemaIndex[]; columnOptions: string[]; onAdd: (x: SchemaIndex) => void; onEdit: (originalName: string, x: SchemaIndex) => void; onDrop: (n: string) => void}) {
...
function RelationEditor({relations, columns, onAdd, onDrop}: {relations: SchemaForeignKey[]; columns: string[]; onAdd: (x: SchemaForeignKey) => void; onDrop: (n: string) => void}) {
  ...
  return <div className="fields-grid relation-grid">
    ...
    {relations.map((x) => <div className="field-row relation-row" key={x.name}>
      ...
      <button className="icon-button danger" type="button" aria-label={`删除外键 ${x.name}`} onClick={() => onDrop(x.name)}><Icon name="trash"/></button>
    </div>)}
    <div className="field-row relation-row relation-add-row">
